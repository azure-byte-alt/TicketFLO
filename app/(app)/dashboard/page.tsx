  ...
  return (
    <div className="p-7 min-h-full">
      ...
      <div className="flex items-start justify-between mb-5">
        ...
        <Link
          href="/practice"
          className="bg-emerald-700 hover:bg-emerald-800 text-white text-[13px] font-semibold px-5 py-2.5 rounded-lg shadow-sm hover:shadow-md transition-all duration-200 whitespace-nowrap no-underline"
        >
          + Start Practice
        </Link>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-xl px-5 py-4 mb-4">
        ...
        <p className="text-[11px] text-gray-400 mt-2">
          {ticketsToUnlock > 0
            ? `Complete ${ticketsToUnlock} more ticket${ticketsToUnlock !== 1 ? 's' : ''} to unlock your certificate`
      ...
      <div className="grid grid-cols-4 gap-3 mb-4">
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-md transition-shadow duration-200">
          ...
          <div className="text-xs text-gray-500 mt-1.5">
            {totalTickets === 0 ? 'Write your first ticket today' : `Start ticket #${totalTickets + 1} today`}
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-md transition-shadow duration-200">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-3" style={{ background: avgStyle.bg }}>
            <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
              <path d="M8 2l1.5 3 3.5.5-2.5 2.5.5 3.5L8 10l-3 1.5.5-3.5L3 5.5l3.5-.5L8 2z" stroke={avgStyle.color} strokeWidth="1.3"/>
            </svg>
          </div>
          ...
          <div className="text-xs mt-1.5" style={{ color: avgStyle.color }}>
            {avgScore >= 80 ? 'Excellent work!' : avgScore >= 60 ? 'Good — keep pushing' : "Below 60 — let's fix that"}
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-md transition-shadow duration-200">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-3 bg-amber-50">
            <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
              <path d="M8 2l1.5 3 3.5.5-2.5 2.5.5 3.5L8 10l-3 1.5.5-3.5L3 5.5l3.5-.5L8 2z" stroke="#b45309" strokeWidth="1.3"/>
            </svg>
          </div>
          ...
          <div className="text-xs text-gray-500 mt-1.5">
            {bestScore >= 80 ? 'Outstanding!' : bestScore >= 60 ? 'Push past 80 next time' : 'Push past 60 next time'}
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-md transition-shadow duration-200">
          <div className="w-8 h-8 rounded-lg flex items-center justify-center mb-3 bg-gray-50">
            <svg width="15" height="15" viewBox="0 0 16 16" fill="none">
              <path d="M9 2C7 4 6 6 8 8c-1 0-2.5-.5-3-2C4.5 8 4 9.5 4 11c0 2.2 1.8 4 4 4s4-1.8 4-4c0-3-2-5-3-9z" stroke="#9ea8b8" strokeWidth="1.3" strokeLinejoin="round"/>
            </svg>
          </div>
          ...
          <div className="text-xs text-gray-500 mt-1.5">
            {streak === 0 ? 'Start your streak today' : `${streak} Day Momentum Streak 🔥`}
          </div>
        </div>
      </div>
        ...
        <div className="bg-white border border-gray-100 rounded-xl p-5">
          ...
          {recentSubmissions.length === 0 ? (
            <div className="text-center py-8">
              ...
              <Link href="/practice" className="text-[13px] text-emerald-700 font-medium hover:underline mt-1 block no-underline">
            ...
            recentSubmissions.map((item, i) => {
              ...
              const title = (item.submissions as any)?.title ?? `Practice Ticket #${totalTickets - i}`
              return (
                <div key={i} className={`flex items-center justify-between py-3 ${i < recentSubmissions.length - 1 ? 'border-b border-gray-50' : ''}`}>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-xs font-bold px-2.5 py-1 rounded-full" style={{ color: style.color, background: style.bg }}>
                      {item.score}/100
                    </span>
                    <Link href="/feedback" className="text-[12px] text-gray-300 hover:text-gray-500 transition-colors no-underline">
                      View →
                    </Link>
                  </div>
                </div>
          ...
          <div className="mt-3">
            <Link href="/feedback" className="text-[12px] text-emerald-700 font-medium hover:underline no-underline">
              View all feedback →
            </Link>
          </div>
        </div>
